...
#
# Muzi가 나간후 다시 들어올 때, Prodo 라는 닉네임으로 들어올 경우 기존에 채팅방에
# 남아있던 Muzi도 Prodo로 다음과 같이 변경된다.
#
...
def record(input:list):

    id_dict = {}    # key - id , value - name
    action_list = [] # action - list

    for record in input:

        splits = record.split(" ")
        action_list.append([splits[1], splits[0]])

        if len(splits) == 3:
            if id_dict.__contains__(splits[1]):
                id_dict[splits[1]] = splits[2]
            else:
                id_dict[splits[1]] = splits[2]
    ...
    for id, action in action_list:
        ...
        if action == "Enter":
            log = "{}님이 들어왔습니다.".format(id_dict[id])
            logs.append(log)
        elif action == "Change":
            pass
        else:
            log = "{}님이 나갔습니다.".format(id_dict[id])
            logs.append(log)
...
output_list = ["Prodo님이 들어왔습니다.", "Ryan님이 들어왔습니다.", "Prodo님이 나갔습니다.", "Prodo님이 들어왔습니다."]

print(record(input_list))
